...
public class CompanyInfoPictureDpl {

public ArrayList<CompanyInfoPicture> excuteQuery(String fSqlString) {
		
		PooledConnection pConn = null;
		ResultSet rSet = null;
		
		ArrayList<CompanyInfoPicture> pictures = null;
	 ...
	 */
	private ArrayList<CompanyInfoPicture> rsetToContainer(
			ResultSet  fRSet) throws Exception {
		
		ArrayList<CompanyInfoPicture> pictures = new ArrayList<CompanyInfoPicture>();
		
		//循环遍历结果集获取数据
		while (fRSet.next()) {
			
			CompanyInfoPicture aPicture = new CompanyInfoPicture();
			
			aPicture.setPictureID(fRSet.getInt("PictureID"));
			aPicture.setInfoID(fRSet.getInt("InfoID"));
