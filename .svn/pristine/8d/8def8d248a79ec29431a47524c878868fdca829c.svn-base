...
public class PageBottomInfoDpl {

public ArrayList<PageBottomInfo> excuteQuery(String fSqlString) {
	 ...
	 */
	private ArrayList<PageBottomInfo> rsetToContainer(
			ResultSet  fRSet) throws Exception {
		
		ArrayList<PageBottomInfo> infos = new ArrayList<PageBottomInfo>();
		
		//循环遍历结果集获取数据
		while (fRSet.next()) {
			
			PageBottomInfo aInfo = new PageBottomInfo();
			
			aInfo.setInfoID(fRSet.getInt("InfoID"));
			aInfo.setInfoName(fRSet.getString("InfoName"));
			aInfo.setInfoValue(fRSet.getString("InfoValue"));
