...
public class PasswordProtectProblemDpl {
	
public ArrayList<PasswordProtectProblem> excuteQuery(String fSqlString) {
		
		PooledConnection pConn = null;
		ResultSet rSet = null;
		
		ArrayList<PasswordProtectProblem> infos = null;
	 ...
	 */
	private ArrayList<PasswordProtectProblem> rsetToContainer(
			ResultSet  fRSet) throws Exception {
		
		ArrayList<PasswordProtectProblem> infos = new ArrayList<PasswordProtectProblem>();
		
		//循环遍历结果集获取数据
		while (fRSet.next()) {
			
			PasswordProtectProblem aInfo = new PasswordProtectProblem();
			
			aInfo.setProblemID(fRSet.getInt("ProblemID"));
			aInfo.setManagerID(fRSet.getInt("ManagerID"));
			aInfo.setProblem(fRSet.getString("Problem"));
			aInfo.setAnswer(fRSet.getString("Answer"));
