...
public class NewsListDpl {
	 ...
	 * @return					封装好了的数据集合
	 */
public ArrayList<NewsList> excuteQuery(String fSqlString) {
		
		PooledConnection 		pConn 				= null;
		ResultSet 				  		rSet					= null;
		
		ArrayList<NewsList>  	newLists		= null;
	 ...
	 */
	private ArrayList<NewsList> RsetToContainer(
			ResultSet  fRSet) throws Exception {
		
		ArrayList<NewsList> newsLists =
				new ArrayList<NewsList>();
		...
		while (fRSet.next()) {
			...
			aNewsList.setNewsID(fRSet.getInt("NewsID"));
			aNewsList.setNewsTitle(fRSet.getString("NewsTitle"));
			aNewsList.setNewsPublishTime(fRSet.getDate("NewsPublishTime"));
			
			newsLists.add(aNewsList);
		}
