...
<script type="text/javascript">
	$(function() {
		...
		function updatePreview(c) {
			if (parseInt(c.w) > 0) {
				$('#preview').css({
					width : Math.round(285 / c.w * boundx) + 'px',		//285*285分辨率的缩略图
					height : Math.round(285 / c.h * boundy) + 'px',
					marginLeft : '-' + Math.round(285 / c.w * c.x) + 'px',
					marginTop : '-' + Math.round(285 / c.h * c.y) + 'px'
				});
...
<div id="main">
	<%
		...
		int viewImgWidth = 285; //缩略图展示宽		//比例1：1
		int viewImgHeight = 285;//缩略图展示的长
		//存在原图地址属性则展示此模块
	...
	%>
	<div class="secondDiv">
	<h2>企业图片剪裁：<span>（如大小不合适可先用图像处理软件处理，最佳尺寸为285像素*285像素）</span></h2>
	<div class="bigPic">
		<span>原图：</span>
	...
	<form action="PicCrop3" method="post">
