...
public class MappingManagerPermissionDpl {

	
	public ArrayList<Mapping_Manager_Permission> excuteQuery(String fSqlString) {
		
		PooledConnection pConn = null;
		ResultSet rSet = null;

		ArrayList<Mapping_Manager_Permission> mappings = null;
	 ...
	 */
	private ArrayList<Mapping_Manager_Permission> rsetToContainer(
			ResultSet  fRSet) throws Exception {
		
		ArrayList<Mapping_Manager_Permission> mappings =
				new ArrayList<Mapping_Manager_Permission>();
		
		//循环遍历结果集获取数据
		while (fRSet.next()) {
			
			Mapping_Manager_Permission aMappings = 
					new Mapping_Manager_Permission();
			
			aMappings.setMappingID(fRSet.getInt("MappingID"));
			aMappings.setManagerID(fRSet.getInt("ManagerID"));
			aMappings.setPermission(fRSet.getString("Permission"));
			
			mappings.add(aMappings);
		}
