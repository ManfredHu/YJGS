...
public class PermissionApplyDpl {

	public ArrayList<PermissionApply> excuteQuery(String fSqlString) {
	 ...
	 */
	private ArrayList<PermissionApply> rsetToContainer(
			ResultSet  fRSet) throws Exception {
		
		ArrayList<PermissionApply> applies = new ArrayList<PermissionApply>();
		...
		while (fRSet.next()) {
			...
			apply.setPerApplyID(fRSet.getInt("PerApplyID"));
			apply.setManagerID(fRSet.getInt("ManagerID"));
			apply.setApplyTime(fRSet.getDate("ApplyTime"));
			
			if(fRSet.getInt("IsRead") == 0) {
				apply.setRead(false);
			}
			else {
				apply.setRead(true);
