...
public class OtherPictureDpl {

	public ArrayList<OtherPicture> excuteQuery(String fSqlString) {
		
		PooledConnection pConn = null;
		ResultSet rSet = null;
		
		ArrayList<OtherPicture> pictures = null;
	 ...
	 */
	private ArrayList<OtherPicture> rsetToContainer(
			ResultSet  fRSet) throws Exception {
		
		ArrayList<OtherPicture> pictures = new ArrayList<OtherPicture>();
		
		//循环遍历结果集获取数据
		while (fRSet.next()) {
			
			OtherPicture aPicture = new OtherPicture();
			...
			aPicture.setPitureType(fRSet.getInt("PictureType"));
