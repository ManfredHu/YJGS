...
/**
 * Servlet implementation class InsertCompanyRecruitmentSel
 */
public class InsertCompanyRecruitmentSel extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public InsertCompanyRecruitmentSel() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(text==""){
			...
			request.setAttribute("URL", "../CompanyInfoManage/UpdateRecruitment.jsp");
			request.getRequestDispatcher("../Master/handlePage.jsp").forward(request, response);
			return;
		}
		
		companyInfo.setInfoType(cit.RECRUITMENT);
		companyInfo2.setInfoType(cit.RECRUITMENT);
		...
		try {
			...
			if(companyInfo2!=null){
				...
				request.setAttribute("text", "企业招聘信息已存在，请前往修改！");
				request.setAttribute("URL", "../CompanyInfoManage/UpdateRecruitment.jsp");
				request.getRequestDispatcher("../Master/handlePage.jsp").forward(request, response);
				return;
			}
			...
			if(cbll.AddText(companyInfo)){
				request.setAttribute("judge", "true");
				request.setAttribute("text", "添加企业招聘信息成功！");
				request.setAttribute("URL", "../CompanyInfoManage/UpdateRecruitment.jsp");
				request.getRequestDispatcher("../Master/handlePage.jsp").forward(request, response);
				return;
			}
			...
			System.out.println("添加企业招聘信息失败！");
			e.printStackTrace();
			request.setAttribute("judge", "false");
			request.setAttribute("text", "添加企业招聘信息失败！");
			request.setAttribute("URL", "../CompanyInfoManage/UpdateRecruitment.jsp");
			request.getRequestDispatcher("../Master/handlePage.jsp").forward(request, response);
			return;
		}
