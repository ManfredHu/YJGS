...
function goTopEx() {
	...
	function getScrollTop() {
		return document.documentElement.scrollTop || document.body.scrollTop; //不用+用||
	}
	...
	obj.onclick = function() {
		var goTop = setInterval(scrollMove, 10);//每10MS执行一次函数
		function scrollMove() {
			setScrollTop(getScrollTop() / 1.1);//每次执行函数递减一定的量，可以理解为页面滚动速度
			if (getScrollTop() < 1)
				clearInterval(goTop);//到达顶部清除计时器
		}
	}
}
/** ******************************** */
