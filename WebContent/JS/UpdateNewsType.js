...
//更新类别	
		var canModify = 1;
	var curTypeCache = "";
	function updateType(thisA, typeID) {
		...
		//LinkYes.setAttribute("onclick", "function goto(){window.location.href='UpdateNewsTypeSel?id="+typeID+"'&newName='"+TextBox.value+"';}");
		...
		TDs.item(2).replaceChild(LinkYes, TDs.item(2).firstChild);
		TDs.item(2).appendChild(LinkNo);
	...
	function Cancel(thisA, typeID) {
		...
		LinkEdit.setAttribute("onclick", "updateType(this," + typeID
				+ ");return false");

		TDs.item(2).replaceChild(LinkEdit, TDs.item(2).firstChild);
		TDs.item(2).removeChild(TDs.item(2).lastChild);
	}
		
		//类别删除函数
		function deleteType(fID){
			//页面跳转
			window.location.href="DeleteNewsTypeSel?id="+fID+"";
		}
		function altColor() {
