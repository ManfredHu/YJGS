...
(function($) {
	$.fn.MobileSlider = function(settings) {
		var defaultSettings = {
			width : 900, // 容器宽度
			height : 325, // 容器高度MobileSlider
			during : 5000, // 间隔时间MobileSlider
			speed : 30
		// 滑动速度
		}
		...
		return this
				.each(function() {
					...
					$("div", oFocusContainer).width(
							oFocus.outerWidth(true) * num).css({
						position : 'absolute',
						right : 10,
						top : '50%',
					...
					$(window)
							.bind(
									...
									function() {
										...
										oLi.width(_this.width()).height(
												_this.height());// 设定滚动单元宽高
										oMover.width(num * oLi.width());
										oFocusContainer.width(_this.width())
												.height(_this.height() * 0.15)
												.css({
													zIndex : 2
												});// 设定焦点容器宽高样式
										_this.fadeIn(300);
					...
					function doMove() {
						...
						oFocus.eq(iCurr).addClass("current").siblings()
								.removeClass("current");
					}
					// 绑定触摸事件
					function bindTochuEvent() {
						oMover.get(0).addEventListener('touchstart',
								touchStartFunc, false);
						oMover.get(0).addEventListener('touchmove',
								touchMoveFunc, false);
						oMover.get(0).addEventListener('touchend',
								touchEndFunc, false);
					}
					// 获取触点位置
					function touchPos(e) {
